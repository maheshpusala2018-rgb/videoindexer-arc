...
# Ensure kubectl uses the AKS Edge kubeconfig instead of localhost fallback.
$kubeDir = Join-Path $env:USERPROFILE ".kube"
$defaultKubeConfig = Join-Path $kubeDir "config"
if (-not (Test-Path $kubeDir)) {
    New-Item -Path $kubeDir -ItemType Directory -Force | Out-Null
}

if (-not (Test-Path $defaultKubeConfig)) {
    Write-Host "Waiting for kubeconfig to be generated..."
    $maxWaitSeconds = 300
    $elapsed = 0
    while ((-not (Test-Path $defaultKubeConfig)) -and ($elapsed -lt $maxWaitSeconds)) {
        Start-Sleep -Seconds 10
        $elapsed += 10
    }

    if (-not (Test-Path $defaultKubeConfig)) {
        $latestKubeConfig = Get-ChildItem -Path $kubeDir -Filter "config*" -File -ErrorAction SilentlyContinue |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 1

        if ($null -ne $latestKubeConfig) {
            Copy-Item -Path $latestKubeConfig.FullName -Destination $defaultKubeConfig -Force
        }
    }
}
...
if (-not (Test-Path $Env:KUBECONFIG)) {
    throw "kubeconfig was not found under $kubeDir. Cannot continue kubectl operations."
}
...
### Video Indexer setup
#####################################################################
$viApiVersions=@("2025-04-01", "2025-03-01", "2025-01-01", "2024-01-01")
...
$apiVersionInUse = $null
foreach ($candidateApiVersion in $viApiVersions) {
    $createResourceUri = "https://management.azure.com/subscriptions/${env:subscriptionId}/resourceGroups/${env:resourceGroup}/providers/Microsoft.VideoIndexer/accounts/${env:videoIndexerAccountName}/CreateExtensionDependencies?api-version=${candidateApiVersion}"
    az rest --method post --uri $createResourceUri 2>$null | Out-Null
    if ($LASTEXITCODE -eq 0) {
        $apiVersionInUse = $candidateApiVersion
        break
    }
}

if ($null -eq $apiVersionInUse) {
    throw "Unable to initialize Video Indexer extension dependencies with supported API versions: $($viApiVersions -join ', ')."
}

$getSecretsUri="https://management.azure.com/subscriptions/${env:subscriptionId}/resourceGroups/${env:resourceGroup}/providers/Microsoft.VideoIndexer/accounts/${env:videoIndexerAccountName}/ListExtensionDependenciesData?api-version=$apiVersionInUse"
while ($null -eq $csResourcesData) {
    Write-Host "Retrieving Cognitive Service Credentials using API version $apiVersionInUse..."
...
$availableExtensionTypes = az k8s-extension extension-types list-by-cluster `
    --cluster-type connectedClusters `
    --cluster-name $clusterName `
    --resource-group $Env:resourceGroup `
    --query "[].name" -o tsv

$videoIndexerExtensionType = $availableExtensionTypes |
    Where-Object { $_ -match "videoindexer|video-indexer" } |
    Select-Object -First 1

if ([string]::IsNullOrWhiteSpace($videoIndexerExtensionType)) {
    throw "Video Indexer extension type is not available for cluster '$clusterName' in resource group '$($Env:resourceGroup)'."
}

$extensionCreateArgs = @(
    "k8s-extension", "create",
    "--name", $extensionName,
    "--extension-type", $videoIndexerExtensionType,
    "--scope", "cluster",
...
if (-not [string]::IsNullOrWhiteSpace($version)) {
    $extensionCreateArgs += @("--version", $version)
}

az @extensionCreateArgs

$extensionState = az k8s-extension show `
    --name $extensionName `
    --cluster-name $clusterName `
    --resource-group $Env:resourceGroup `
    --cluster-type connectedClusters `
...
if ($extensionState -ne "Succeeded") {
    throw "Video Indexer extension provisioning state is '$extensionState'."
}
